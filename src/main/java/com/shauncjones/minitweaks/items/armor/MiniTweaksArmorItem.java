...
public class MiniTweaksArmorItem extends ArmorItem {
    private static final Map<IArmorMaterial, Effect> MATERIAL_TO_EFFECT_MAP =
            new ImmutableMap.Builder<IArmorMaterial, Effect>()
                    .put(MiniTweaksArmorMaterial.GOGGLES, Effects.NIGHT_VISION)
                    .build();

    public MiniTweaksArmorItem(IArmorMaterial material, EquipmentSlotType slot, Properties settings) {
    ...
    public void onArmorTick(ItemStack stack, World world, PlayerEntity player) {
        if(!world.isRemote()) {
            ...
            }else if(player.inventory.armorItemInSlot(3).getItem() == MiniTweakItems.GOGGLES.get()){
                evaluateArmorEffects(player);
            }
        }
